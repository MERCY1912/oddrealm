...
-- 6. RLS политики (если нужно ограничить доступ)
-- Пока оставляем открытый доступ для разработки
...
-- 11. Добавляем ботов в user_activity для отображения в онлайн списке
INSERT INTO public.user_activity (user_id, last_seen, status, location, last_activity)
SELECT 
    'bot_' || bc.id::text::uuid as user_id,
